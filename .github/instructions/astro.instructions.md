...
- Define and document a `Props` interface in every reusable component. The interface comment should describe the component contract and its properties, including inherited HTML attributes when applicable.
...
- Comment why a component or non-obvious block exists, not what self-explanatory markup already does.
- Keep component contract comments current when props change. Remove comments that become inaccurate instead of leaving them as historical notes.
